...
# ===========================================================================
if [[ $# -lt 3 || $# -gt 4 ]];then
    echo "Usage1: bash run_distribute_train.sh [CONFIG_PATH] [RANK_TABLE_FILE] [DEVICENUM]"
exit 1
fi
...
then
  ...
  do
      ...
      cp -r ./yolox_tools ./train_parallel$i
      cp -r ./third_party ./train_parallel$i

      cd ./train_parallel$i || exit
      echo "start training for rank $RANK_ID, device $DEVICE_ID"
      env > env.log
      taskset -c $cmdopt python yolox_tools/train.py \
          --config=$CONFIG_PATH\
