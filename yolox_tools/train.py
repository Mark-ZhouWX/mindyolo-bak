...
from yolox_tools.device_adapter import get_device_id, get_device_num
from yolox_tools.moxing_adapter import moxing_wrapper
from yolox_tools.initializer import default_recurisive_init
from yolox_tools.logger import get_logger
from mindyolo.models.layers.yolox_blocks import use_syc_bn
...
def set_callback(cfg, ds=None, backbone='yolofpn'):
    ...
    if cfg.run_eval and ds is not None:
        test_network = YOLOx(cfg, backbone=backbone)
        save_prefix = None
...
def run_train(cfg):
    ...
    if cfg.backbone == "yolox_darknet53":
        backbone = "yolofpn"
    elif cfg.backbone in ['yolox_x', 'yolox_s']:
        backbone = "yolopafpn"
    else:
        raise ValueError('backbone only support [yolox_darknet53, yolox_x]')
    base_network = YOLOx(cfg, backbone=backbone)
